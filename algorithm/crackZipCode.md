...
当你拿到一个带密码的压缩包时，要让加密者强制遵守开源精神，就需要我们自己破解它

在此之前，需要明白加密者对压缩包做了什么？



要使得小黄油解压后和压缩前，连毛都一样，就需要使用无损压缩

常用**LZ算法**：找到文件中重复的结构，并将它们简化

举个栗子：假设有一个文本文件，其中包含 “嗡嗡嗡嗡嗡”，LZ算法会将第一个之后的“嗡”替换为对第一个“嗡”的引用，从而减少占用的空间。



而最重要的加密阶段，发生在数据压缩之后，但在将数据写入最终 ZIP 文件之前。

可以使用我们小学二年级就学过的AES（Advanced Encryption Standard）

**它是目前国际上广泛应用的加密标准**，你家的路由器也用着这个标准，对你的上网流量进行加密。
...
也就是将ikun520变为ASCII编码，根据PKCS7填充标准，填充为16字节

最终转化为十六进制：69 6B 75 6E 35 32 30 09 09 09 09 09 09 09 09 09（其中 09 表示填充的字节）

另外准备初始密钥

AES-128 使用 128 位密钥，由“密钥派生函数KDF”生成

这里我们假设初始密钥为：2b7e151628aed2a6abf7158809cf4f3c。这将被用于后续操作。
...
类似于将中文翻译成对应的英文单词，并将英文单词按4行4列，排列整齐



接下来是行移位

这一步会基于一定的规律，移动矩阵中的每一行

得到

D0 BD AC B2（**第 1 行**保持不变）

30 50 3D 18（**第 2 行**向左循环移位 1 个字节）

3D 3D 3D 3D（**第 3 行**向左循环移位 2 个字节；这里所有字节相同，移位后不变）

3D 3D 3D 3D（**第 4 行**向左循环移位 3 个字节；同样因为字节相同，移位后不变）



第三步是列混合

将上一步得到的矩阵与一个固定的4x4矩阵相乘
...
其中，轮密钥是根据初始密钥，使用AES密钥扩展算法来生成的，具体算法因为篇幅原因，不做解释。

初始密钥经过密钥扩展算法

得到第一轮密钥a0fafe1788a4ccb12393d9292adf96a5
...
暴力破解指的是通过逐一尝试所有可能的密码组合来猜测出正确密码
...
破解时长根据密码难易程度决定，测试之前的加密压缩包，十秒就可以搞定，非常带劲。



所有的链接已经放到视频简介中了，可以收藏下来，以备不时之需。
